...
nextGreen = [
    (cg + 1) % noOfSignals for cg in currentGreen
...
def reset_stops(direction_index):
    for i in range(0, 3):
        for vehicle in vehicles[directionNumbers[direction_index]][i]:
            vehicle.stop = defaultStop[directionNumbers[direction_index]]
...
def repeat():
    ...
    ):  # While any green timer is active
        updateValues()
        time.sleep(1)

    currentYellow = 1  # set yellow signal on

    [reset_stops(cg) for cg in currentGreen]  # Apply reset_stops to each direction

    while (
        signals[currentGreen[0]].yellow > 0
